...
def has_bet_bear(epoch):
    # Check if the user has already placed a betBear for the given epoch
    try:
        bet_info = contract.functions.ledger(epoch, public_address).call()
        return bet_info[1] > 0  # Assuming the second element in bet_info indicates betBear
    except Exception as e:
        print(f"Error checking if betBear is placed for epoch {epoch}: {e}")
        return False
...
try:
    while True:
        ...
        if bet_placed_epoch != current_epoch and not has_bet(current_epoch) and not has_bet_bear(current_epoch):
